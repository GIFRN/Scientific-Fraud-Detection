...
def flatten(y_true, y_pred):
    """Flatten sequences for evaluation, removing padding."""
    y_true_flat = []
    y_pred_flat = []
    for true_seq, pred_seq in zip(y_true, y_pred):
        for t, p in zip(true_seq, pred_seq):
            if t != 0:  # Skip padding (assuming 0 is padding)
                y_true_flat.append(t)
                y_pred_flat.append(p)
    return np.array(y_true_flat), np.array(y_pred_flat)


def classification_report(y_true, y_pred, target_names=None, labels=None, digits=4, output_dict=False):
    """Wrapper for sklearn classification_report that handles sequence data."""
