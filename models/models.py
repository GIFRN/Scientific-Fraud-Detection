...
def _softmax(x, dim):
    ...
    elif K.backend() == 'cntk':  # Note: CNTK backend is deprecated
        import cntk
...
def bert_transformer(sents_shape, num_classes, transformer_params):
    """
    :param sents_shape: (num_sents, sent_maxlen)
    :param num_classes: int, number of classes
    :param transformer_params: dict, contains dropout and recurrent_dropout
    :return: keras.models.Model
    """
    inputs = Input(shape=sents_shape)
    ...
    sent_scores = TimeDistributed(output_layer)(x)

    model = Model(inputs=inputs, outputs=sent_scores)
    model.compile(optimizer=Adam(learning_rate=0.0001, beta_1=0.9, beta_2=0.98),
...
def bert_transformer_layers(sents_shape, num_classes, transformer_params):
    inputs = tf.keras.Input(shape=sents_shape)

    # Increase the number of Transformer blocks
    prev_output = inputs
    for _ in range(transformer_params['num_layers']):
        transformer_block = TransformerBlockGelu(
            embed_dim=transformer_params['embed_dim'],
            num_heads=transformer_params['num_heads'],
            ff_dim=transformer_params['ff_dim'],
            rate=transformer_params['dropout_rate']
        )
        prev_output = transformer_block(prev_output)

    output_layer = layers.Dense(num_classes, activation='softmax')
    sent_scores = layers.TimeDistributed(output_layer)(prev_output)

    model = Model(inputs=inputs, outputs=sent_scores)
    
    # Change to learning rate scheduler
    learning_rate = CustomSchedule(transformer_params['embed_dim'])

    # Use TensorFlow's built-in AdamW optimizer (available in TF 2.11+)
